...
bool relayFans = false;
bool relayPump = false;

// Relay polling
unsigned long lastRelayPoll = 0;
const unsigned long RELAY_POLL_INTERVAL = 5000; // Check every 5 seconds
...
void applyRelayStates() {
  setRelay(RELAY_LIGHTS_PIN, relayLights);
  setRelay(RELAY_FANS_PIN, relayFans);
  setRelay(RELAY_PUMP_PIN, relayPump);
}

void fetchRelayStates() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[RELAY] SKIP: WiFi disconnected");
    return;
  }
  ...
  if (code == 200) {
    ...
    if (!error) {
      ...
      bool newLights = (strcmp(relays["lights"] | "OFF", "ON") == 0);
      bool newFans = (strcmp(relays["fans"] | "OFF", "ON") == 0);
      bool newPump = (strcmp(relays["pump"] | "OFF", "ON") == 0);

      // Check for changes and log
      if (newLights != relayLights || newFans != relayFans || newPump != relayPump) {
        Serial.println("[RELAY] State change detected:");
        if (newLights != relayLights) {
          Serial.print("  Lights: "); Serial.print(relayLights ? "ON" : "OFF");
          Serial.print(" -> "); Serial.println(newLights ? "ON" : "OFF");
        }
        if (newFans != relayFans) {
          Serial.print("  Fans: "); Serial.print(relayFans ? "ON" : "OFF");
          Serial.print(" -> "); Serial.println(newFans ? "ON" : "OFF");
        }
        if (newPump != relayPump) {
          Serial.print("  Pump: "); Serial.print(relayPump ? "ON" : "OFF");
          Serial.print(" -> "); Serial.println(newPump ? "ON" : "OFF");
        }

        // Update states
        relayLights = newLights;
        relayFans = newFans;
        relayPump = newPump;
    ...
    Serial.print("[RELAY] Server error: ");
    Serial.println(code);
  } else {
    Serial.print("[RELAY] Connection error: ");
    Serial.println(http.errorToString(code));
  }
...
void setup() {
  ...
  // Initialize relay pins (start OFF - HIGH for active LOW relays)
  pinMode(RELAY_LIGHTS_PIN, OUTPUT);
  pinMode(RELAY_FANS_PIN, OUTPUT);
  pinMode(RELAY_PUMP_PIN, OUTPUT);
  digitalWrite(RELAY_LIGHTS_PIN, HIGH);  // OFF
  digitalWrite(RELAY_FANS_PIN, HIGH);    // OFF
  ...
  // Fetch initial relay states from server
  Serial.println("[RELAY] Fetching initial states...");
  fetchRelayStates();
